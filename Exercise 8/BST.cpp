...
{
  root = copyBST(bst.root); // becomes new root to new tree
}
...
//Recursive helper fcn that returns a BST node
TreeNode* BST::copyBST(const TreeNode* original)
{
  if(original == nullptr)
  {
    return nullptr;
  }
  TreeNode* copy = new TreeNode(original->data);
  copy->left = copyBST(original->left);
  copy->right = copyBST(original->right);
  return copy;
}
